...
import tw.brad.springdemo.dto.BookingResponseDTO;
import tw.brad.springdemo.dto.CancelResponseDTO;
import tw.brad.springdemo.dto.CourseDto;
import tw.brad.springdemo.dto.PackageResponseDTO;
import tw.brad.springdemo.dto.TodayCourseDto;
...
public class StudentCourseService {
	 ...
	 @Autowired private BookingsRepo bookingsRepo;
	 @Autowired private CoursesRepo coursesRepo; // needed for purchase logic

    // 取得我的課程包 (Orders)
    public List<PackageResponseDTO> getMyPackages(Long userId) {
        return ordersRepo.findByUser_Id(userId).stream()
    ...
    public PackageResponseDTO Buycourse(Long studentId, Long courseId, Integer lessonCount) {
        ...
        // 2. 加老師錢包
        Users tutor = course.getTutor();
        tutor.setWallet(tutor.getWallet() + totalPrice);
        usersRepo.save(tutor);
        ...
        order.setStatus((byte) 1); // 1 = 有效/進行中
        ordersRepo.save(order);

        // 4. 建立錢包日誌：學生消費
        WalletLogs studentLog = new WalletLogs();
        studentLog.setUser(student);
        studentLog.setTransactionType((byte) 2); // 2 = 購課
        studentLog.setAmount(totalPrice); // 金額欄位始終正數，由 transactionType 區分出入
        studentLog.setRelatedType((byte) 1); // 1 = order
        studentLog.setRelatedId(order.getId());
        walletLogsRepo.save(studentLog);

        // 5. 建立錢包日誌：老師收入
        WalletLogs tutorLog = new WalletLogs();
        tutorLog.setUser(tutor);
        tutorLog.setTransactionType((byte) 3); // 3 = 授課收入
        tutorLog.setAmount(totalPrice);
        tutorLog.setRelatedType((byte) 1);
    ...
    public PackageResponseDTO getPackageById(Long packageId) {
        Orders order = ordersRepo.findById(packageId).orElseThrow();
        return new PackageResponseDTO(
            order.getId(),
            order.getCourse().getName(),
            order.getLessonCount(),
    ...
    public CancelResponseDTO cancelBooking(Long bookingId, Long userId) {
        // 1. 查找預約並驗證身分
        Bookings booking = bookingsRepo.findByIdAndStudent_Id(bookingId, userId)
    ...
    public String refundEntireOrder(Long orderId, Long userId) {
        ...
        user.setWallet(user.getWallet() + refundAmount);
	    usersRepo.save(user);
        ...
        log.setUser(user);
        log.setTransactionType((byte) 4); // 4=退款
        log.setAmount(refundAmount);
        log.setRelatedType((byte) 1);    // 1=order
